...
# 输出文件路径
output_file = "gps_truth_trans.txt"
...
def save_to_tum():
    ...
    # 检查是否与过去的数据重复
    is_valid = True
    for past_data in past_gps_data:
        if past_data["tx"] == tx and past_data["ty"] == ty and past_data["tz"] == tz:
            is_valid = False
            break
    ...
    if is_valid:
        ...
        # 更新过去的数据列表
        past_gps_data.append({"tx": tx, "ty": ty, "tz": tz})
        if len(past_gps_data) > 20:
            past_gps_data.pop(0)
    else:
        rospy.loginfo(f"Ignored duplicate data: {timestamp} {tx} {ty} {tz} {qx} {qy} {qz} {qw}")
