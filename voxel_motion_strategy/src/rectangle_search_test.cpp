...
int main(int argc, char** argv) {
  ...
  double p_min = -50.0 * kDeg, p_max = 20.0 * kDeg;
  ...
  std::cout << std::endl << "--- Test 1: full-range search ---" << std::endl;
  {
    ...
    auto result = searchBestRectangle(ii, erp, -180.0 * kDeg, 180.0 * kDeg,
                                       p_min, p_max, sp);
  ...
  std::cout << std::endl << "--- Test 2: constrained yaw range ---" << std::endl;
  {
    ...
    double dt = 0.5;
    double cur_yaw = 50.0 * kDeg;

    auto yr = computeAngleRange(cur_yaw, vel_max, dt);
    CHECK(yr.valid, "yaw range is valid");

    RectSearchParams sp;

    auto result = searchBestRectangle(ii, erp, yr.min, yr.max, p_min, p_max, sp);
  ...
  // Test 3: pitch penalty effect
  std::cout << std::endl << "--- Test 3: pitch penalty effect ---" << std::endl;
  {
    RectSearchParams sp_lo, sp_hi;
    sp_lo.weight_pitch = 0.0;
    sp_hi.weight_pitch = 1.0;

    auto r_lo = searchBestRectangle(ii, erp, -180.0 * kDeg, 180.0 * kDeg,
                                     p_min, p_max, sp_lo);
    auto r_hi = searchBestRectangle(ii, erp, -180.0 * kDeg, 180.0 * kDeg,
                                     p_min, p_max, sp_hi);
  ...
  std::cout << std::endl << "--- Test 4: edge cases ---" << std::endl;
  {
    ...
    ERPImage empty_erp;
    RectSearchParams sp;
    auto r = searchBestRectangle(empty_ii, empty_erp, -1.0, 1.0, -1.0, 1.0, sp);
    CHECK(!r.valid, "empty integral → invalid result");
  }
  {
    RectSearchParams sp;
    auto r = searchBestRectangle(ii, erp, 0.0, 0.0, p_min, p_max, sp);
